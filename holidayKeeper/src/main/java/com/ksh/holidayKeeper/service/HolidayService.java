...
public class HolidayService {
    ...
    public void upsertYearAndCountry(int year, String countryCode) {
        List<Map<String, Object>> holidaysFromApi = nagerApiClient.fetchHolidays(year, countryCode);

        for (Map<String, Object> h : holidaysFromApi) {
            LocalDate holidayDate = LocalDate.parse((String) h.get("date"));

            // countryCode와 date로 기존 데이터가 있는지 확인
            Holiday holiday = holidayRepository.findByCountryCodeAndDate(countryCode, holidayDate)
                    .orElseGet(() -> Holiday.builder() // 없으면 새로 생성
            ...
            holiday.setLocalName((String) h.get("localName"));
            holiday.setName((String) h.get("name"));

            if (h.get("types") instanceof List<?> typesList && !typesList.isEmpty()) {
                holiday.setTypes(String.join(",", typesList.stream().map(String::valueOf).toList()));
            }
    ...
    public HolidayOffsetList searchByOffset(String[] countries, Integer from, Integer to, String type, int page, int size, Sort.Direction direction, String[] sortBy) {
        ...
        if (sortBy == null || sortBy.length == 0) {
            ...
            //#. 정렬 가능한 필드 화이트리스트
            List<String> allowedSortFields = List.of("countryCode", "holidayYear", "name", "localName", "date", "types");
        ...
        Page<Holiday> data = holidayRepository.findHolidayByPage(countries, from, to, type, pageable);
    ...
    public HolidayCursorList searchByCursor(String[] countries, Integer from, Integer to, String type, long cursorId, int size) {
        // 커서 기반 페이징에서는 id 오름차순 정렬이 필수적입니다.
        Sort sort = Sort.by(Sort.Direction.ASC, "id");
        Pageable pageable = PageRequest.of(0, Math.min(size, 100), sort);

        List<Holiday> data = holidayRepository.findHolidayByCursor(countries, from, to, type, cursorId, pageable);
    ...
    private HolidayItem toItem(Holiday a) {
        return new HolidayItem(a.getId(), a.getCountryCode(), a.getHolidayYear(), a.getLocalName(), a.getName(), a.getDate().atStartOfDay().toInstant(ZoneOffset.UTC), a.getTypes());
    }    
}
